...
def validate_ai_coverage(parquet_path, dt_path, output_dir, model_name):
    ...
    round_suffix = f"_round_{os.environ.get('BENCHMARK_ROUND', 'final')}"
    with open(f"{output_dir}/{model_name}_ai_validation_metrics{round_suffix}.txt", "w", encoding="utf-8") as f:
        f.write(report)
    ...
    plt.savefig(f"{output_dir}/{model_name}_ai_validation_plots{round_suffix}.pdf", dpi=300, bbox_inches='tight')
    plt.close(fig)
    ...
    plt.savefig(f"{output_dir}/{model_name}_ai_validation_cdf{round_suffix}.pdf", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"{output_dir}/{model_name}_ai_validation_env_merit{round_suffix}.pdf", dpi=300, bbox_inches='tight')
    plt.close(fig3d)
    
    duration = time.time() - start_time
    print(f"Validação espacial concluída em {duration / 60.0:.2f} minutos")
    return duration
...
def main():
    ...
    # Define diretório central de benchmark se estiver em rodada de benchmark
    is_benchmark = "BENCHMARK_ROUND" in os.environ
    base_results_dir = os.path.join(repo_root, "data", "luiz_costa", "benchmark_spatial_validation") if is_benchmark else None
    ...
    for model in models:
        ...
        if os.path.exists(parquet_file):
            times[model] = validate_ai_coverage(parquet_file, DT_PATH, output_dir, model)
        else:
            times[model] = 0.0
            
    # Salva todos os tempos no arquivo compartilhado temporário
