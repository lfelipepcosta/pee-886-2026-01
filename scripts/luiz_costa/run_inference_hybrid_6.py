...
MODEL_PATH = os.path.join(repo_root, "data", "luiz_costa", "trained_models", "best_pipeline_hybrid_6.joblib")
...
def main():
    ...
    Utiliza o modelo Híbrido Quântico de 6 qubits carregado do disco.
    '''
    print("Iniciando inferência para mapeamento de cobertura (Híbrido Quântico com 6 qubits)")
    ...
    plot_coverage_map(df_grid, df_route, df_antennas, model_name="Hybrid_Quantum_6", target_name="SS-RSRP", output_dir=OUTPUT_DIR)

    # Salva o arquivo de resultados geoespaciais em formato Parquet para validação futura
    output_parquet = os.path.join(OUTPUT_DIR, "ai_coverage_30m_hybrid_6.parquet")
    df_grid[['Latitude', 'Longitude', 'RSRP_dBm']].to_parquet(output_parquet, index=False)
    print("Mapeamento geoespacial híbrido (6 qubits) concluído")
