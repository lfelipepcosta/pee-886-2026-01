...
# Localização do pipeline treinado do Decision Tree e diretório de saída
MODEL_PATH = os.path.join(repo_root, "data", "luiz_costa", "trained_models", "best_pipeline_dt.joblib")
OUTPUT_DIR = os.path.join(repo_root, "data", "luiz_costa", "inference_results")
os.makedirs(OUTPUT_DIR, exist_ok=True)

def main():
    '''
    Gera as predições de cobertura para todos os pontos da malha utilizando o modelo Decision Tree.
    '''
    print("Iniciando inferência para mapeamento de cobertura (Decision Tree)")
    ...
    # Carrega o modelo Decision Tree do disco e executa as predições
    ...
    plot_coverage_map(df_grid, df_route, df_antennas, model_name="Decision Tree", target_name="SS-RSRP", output_dir=OUTPUT_DIR)

    # Persiste os resultados totais em arquivo Parquet
    output_parquet = os.path.join(OUTPUT_DIR, "ai_coverage_30m_dt.parquet")
    df_grid[['Latitude', 'Longitude', 'RSRP_dBm']].to_parquet(output_parquet, index=False)
    print("Mapeamento geoespacial Decision Tree concluído")
