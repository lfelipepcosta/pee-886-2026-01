...
def main():     
    ...
    run_kfold_validation(pipeline, X, y, model_name="MLP", n_splits=5, output_dir=RESULTS_DIR)
    ...
    run_gkfold_validation(pipeline, X, y, groups=groups, model_name="MLP", n_splits=5, output_dir=RESULTS_DIR)
    ...
    plot_feature_importance(pipeline, X_test, y_test, target_label, "MLP", output_dir=RESULTS_DIR)
    plot_actual_vs_predicted(y_test, y_pred, target_label, "MLP", output_dir=RESULTS_DIR)
    plot_error_distribution(y_test, y_pred, target_label, "MLP", output_dir=RESULTS_DIR)
