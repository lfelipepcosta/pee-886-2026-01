...
def validate_ai_coverage(parquet_path, dt_path, output_dir, model_name):
    ...
    for i, (act, pred, err, rmse, title) in enumerate(plot_configs):
        ...
        ax_scat.set_title(f"{title}\nRMSE: {rmse:.2f} dB", fontsize=28, fontweight='bold')
        ax_scat.set_xlabel("Medição RSRP real (dBm)", fontsize=24)
        ax_scat.set_ylabel("Predição RSRP (dBm)", fontsize=24)
        ...
        ax_hist.set_title("Distribuição do Erro Residual", fontsize=28)
        ax_hist.set_xlabel("Erro (Predito - Real) [dB]", fontsize=24)
        ax_hist.set_ylabel("Frequência", fontsize=24)
    ...
    for err, label, color in zip([raw_errors, calib_errors], ['Bruto', 'Calibrado'], ['gray', 'green']):
        ...
        plt.text(p90_val, 0.05, f"P90: {p90_val:.1f}dB", color=color, rotation=90, verticalalignment='bottom', fontweight='bold', fontsize=24)

    plt.axhline(y=0.90, color='red', linestyle='dotted', alpha=0.7, label='Percentil 90')
    plt.title(f"CDF do Erro Absoluto - {model_name}", fontsize=28, fontweight='bold')
    plt.xlabel("Erro Absoluto |dB|", fontsize=24)
    plt.ylabel("Probabilidade Acumulada", fontsize=24)
    plt.xlim([0, 50])
    plt.ylim([0, 1.05])
...
def main():
    '''
    Loop principal para validar sequencialmente os modelos XGB, MLP e Híbrido com 4 qubits.
    '''
    models = ["dt", "xgb", "mlp", "hybrid_4"]
    for model in models:
